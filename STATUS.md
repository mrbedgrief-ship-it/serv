...
- File checks for Java, server jar, datapack, EULA state, and local login mode.
...
- Local testing mode uses `online-mode=false` so username verification does not block join attempts.
...
- Public online account verification for a real internet-facing server.
...
- Offline testing mode is only for local use. Do not expose this build to the public internet.
