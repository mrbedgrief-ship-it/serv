...
function Test-CripCraftFiles {
    ...
    if (Test-Path $runtimeProps) { $messages.Add('Runtime server folder: READY') } else { $messages.Add('Runtime server folder: Will be created on first Play click.') }
...
$playButton.Add_Click({
    $output.Text = "Starting CripCraft...`r`nA server window should open. Wait for the word DONE, then join localhost in Minecraft Java 1.20.4."
